...
class Model_page extends CI_Model 
{
    ...
    public function update_portfolio($data)
    {
     ...
     public function show_photo_galery()
    {
        $query = $this->db->query("SELECT * from tbl_page_photo_gallery WHERE id=1");
        return $query->first_row('array');
    }

     public function update_photo_galery($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_photo_gallery',$data);
    }

     public function show_represented()
    {
        $query = $this->db->query("SELECT * from tbl_page_represented WHERE id=1");
        return $query->first_row('array');
    }

    public function update_represented($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_represented',$data);
    }

    public function show_icons()
    {
        $query = $this->db->query("SELECT * from tbl_icons");
