...
class Model_home extends CI_Model 
{
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_service WHERE status = 1 ORDER BY id_order ASC");
        return $query->result_array();
    }
    ...
    public function all_menu_home()
    {
        $query = $this->db->query("SELECT * FROM tbl_page_home", array('Yes'));
        return $query->result_array();
    }

    public function all_represented()
    {
        $query = $this->db->query("SELECT * FROM tbl_represented", array('Yes'));
        return $query->result_array();
    }
}