	...
	public function update()
	{
		...
		if(isset($_POST['form_logo'])) {
		    ...
		    if($path!='') {
		        ...
		        if($ext_check == FALSE) {
		            $valid = 0;
		            $error = 'Você deve fazer o upload de arquivos jpg, jpeg, gif ou png<br>';
		        }
		    } else {
		    	$valid = 0;
		        $error = 'Você deve selecionar uma foto<br>';
		    }
		    if($valid == 1) {
	        	...
	        	$success = 'O logotipo foi atualizado com sucesso!';
		...
		if(isset($_POST['form_banner_represented'])) {
			$valid = 1;
			$path = $_FILES['photo']['name'];
		    $path_tmp = $_FILES['photo']['tmp_name'];
		    if($path!='') {
		    ...
		    if($valid == 1) {
		    	unlink('./public/uploads/'.$data['setting']['banner_represented']);
		    	$final_name = 'banner_represented'.'.'.$ext;
		        move_uploaded_file( $path_tmp, './public/uploads/'.$final_name );		    			        
				$form_data = array(
					'banner_represented' => $final_name
	            );
	        	$this->Model_setting->update($form_data);
	        	$success = 'O banner da página da representadas foi atualizado com sucesso!';
		    	$this->session->set_flashdata('success',$success);
		    	redirect(base_url().'admin/setting');
		    } else {
		    	$this->session->set_flashdata('error',$error);
