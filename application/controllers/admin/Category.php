...
class Category extends CI_Controller 
{
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($path!='') {
		        ...
		        if($ext_check == FALSE) {
		            $valid = 0;
		            $error .= 'Você deve fazer o upload de arquivos jpg, jpeg, gif ou png para banner<br>';
		        }
		    } else {
		    	$valid = 0;
		        $error .= 'Você deve selecionar uma foto para banner<br>';
		    }
	...
	public function delete($id) 
	{
    	...
    	if($status) 
    	{
    		$error = 'A categoria não pode ser excluída porque há notícias sob este';
    		$this->session->set_flashdata('error',$error);
    		redirect(base_url().'admin/category');
    	}
    	else
